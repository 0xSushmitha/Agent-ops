    ...
    if isinstance(llm_attr, dict):
        try:
            ...
            if(error!=1):
                ...
                tokenCount_completion= llm_attr["llm"]["token_count"]["prompt"]
        except:
            pass
...
def calcCost(df):
    ...
    def calculate_row_cost(row):
        ...
        if model_info:
            ...
            total_cost = (row['token_input'] * model_info['prompt_cost_per_token']/1000) + \
                         (row['token_output'] * model_info['completion_cost_per_token']/1000)
            return total_cost
        else:
            return 0
